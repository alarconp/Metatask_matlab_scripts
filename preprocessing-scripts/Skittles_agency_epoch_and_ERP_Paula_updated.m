...
triggers = {'26' '42' '6'};
events = {'T1_correct_' 'T1_wrong_' 'T2_'};
...
number_of_subj=1 %Added by Paula: To run for one participant only (now I just have eeg data of one participant)

%for number_of_subj= 1:length(subject_clean) %To loop across participants %Comment out
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 3,'setname','epoched_rbase','gui','off');
        ...
        for x = 1:length(conditions) %To create different datasets for each of the two conditions (mo and ml)
        ...
        [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 4,'setname','epoched_rbase_selected','gui','off');
            ...
            EEG.save_name= [subject_clean{number_of_subj} '_epoch_' event condition]
                if save_everything
...
%end %Ends looping across participants %Comment out