% gtec import function pop_read_gtec
% modified by ANgeliki Charalampaki: i modified some functions to avoid
% popping window and several errors
function [EEG, ALLEEG, command] = pop_read_gtec_angeliki(ALLEEG, filename,filepath)
EEG = [];
command = 'pop_read_gtec(ALLEEG)';

fprintf('g.tec file format import function');

% angeliki: I commented this out because i do not want any window popping
% up. I added the filepath variable in the function input variable 
...
% angeliki: i commented this out because i only upload hdf5 files
...
switch ext
    ...
    case '.hdf5'
        %         angeliki: i commented this out to avoid the window popping up
        ...
        %             filenametmp = 0;
        %         end
         filenametmp = -1; % angeliki. i gave this value to avoid the window poping
        if filenametmp ~= 0
            classinfo_filename = [filepath '/' filenametmp];   
            EEG = read_gtec_hdf5events_Skittles_agency(EEG, filename, classinfo_filename); % angeliki: i use the function i modified to have no window pop up
        elseif filenametmp == -1
            EEG = read_gtec_hdf5events_Skittles_agency(EEG, filename, 0);  % angeliki: i use the function i modified to have no window pop up
        end
