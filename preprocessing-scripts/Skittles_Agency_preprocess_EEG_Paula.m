...
number_of_subj=1 %Added by Paula: Comment out this line, to run for more than one participant (now I just have eeg data for one participant)
...
for number_of_subj= 1:length(subject_clean) %To loop across participants
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, (session(end)+1),'setname',subject_list{number_of_subj},'gui','off'); % I only use this now that i want to analyze specific participant
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 4,'setname','Merged datasets all four eeg','gui','off');
    ...
    EEG = pop_reref( EEG, []);
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 5,'setname','Merged datasets all four eeg_rav','gui','off');
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 6,'setname','Merged datasets all four eeg_rav_lowpass','gui','off');
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 7,'setname','Merged datasets all four eeg_rav_lowpass_notch','gui','off');
