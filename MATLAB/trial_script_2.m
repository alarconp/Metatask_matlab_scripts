...
% Data_to_be_saved:
Subject_ID = input('Enter your ID code: ', 's');
participant_trial_sequence = [];
arousal_rating = [];
type_I_RT = [] ;
confidence_rating = [];
type_II_RT = [];
%Skipped/error trials = [];
VariableNames = {'Subject_ID', 'participant_trial_sequence', 'arousal_rating', 'type_I_RT', 'confidence_rating', 'type_II_RT'};

% Create cell array for the participant
Subject_data_table = {};
...
Subject_data_table = {Subject_ID, participant_trial_sequence, arousal_rating, type_I_RT, confidence_rating, type_II_RT};

% Sanity check:
if length(Subject_data_table) ~= length(VariableNames);
    error('Unequal number of variables to save')
end
